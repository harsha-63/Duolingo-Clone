...
const LessonProvider = ({ children }) => {
      ...
      try {
        ...
        if (sectionsResponse.data.length > 0) {
          ...
          if (lessonsResponse.data.length > 0) {
            ...
            const questionsResponse = await axios.get(`http://localhost:4000/user/lessons/${firstLessonId}/questions`);
  ...
  return (
    <LessonContext.Provider value={{ sections, lessons, questions }}>
      {children}
